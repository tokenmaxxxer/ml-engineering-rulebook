...
# gate's own section-placement and pass/bypass word-boundary regressions.
set -uo pipefail
...
GATE_OFF_VAR="ML_ENGINEERING_ML_TEST_SCORE_GATE_OFF"
...
## Monitoring Tests
monitoring alerts bypass the on-call rotation during business hours'

WRONG_SECTION_CONTENT='## Data Tests
pass: data schema validated
...
## ML Infrastructure Tests
pass: infra reproducibility checked (verdict lives in Data Tests, not here)

## Monitoring Tests
not scored in this section, see ML Infrastructure Tests above'

# --- base allow/deny ---
run_write allow all-sections-scored "$PASS_CONTENT"
run_write deny  monitoring-unscored "$FAIL_CONTENT"

# --- pass/bypass word-boundary (Finding 3) ---
run_write deny  bypass-not-pass "$BYPASS_CONTENT"

# --- section-placement (Finding 2 / requirement 2) ---
run_write deny  verdict-in-wrong-section "$WRONG_SECTION_CONTENT"

# --- Edit, single occurrence, section-placed correctly => allow ---
run_edit allow edit-fixes-monitoring "$FAIL_CONTENT" "not yet evaluated" "pass: monitoring checked" ""

# --- Edit with replace_all: true against a multiply-occurring old_string ---
CURRENT_MULTI='## Data Tests
TBD
...
## Monitoring Tests
TBD'
run_edit allow edit-replace-all-true  "$CURRENT_MULTI" "TBD" "pass: checked" "true"
run_edit deny  edit-replace-all-false "$CURRENT_MULTI" "TBD" "pass: checked" "false"

# --- MultiEdit, mixed replace_all true/false per edit, each independently honored ---
CURRENT_MIXED='## Data Tests
X_A
...
## Monitoring Tests
X_B'
EDITS_MIXED_DENY='[{"old_string":"X_A","new_string":"pass: ok","replace_all":true},{"old_string":"X_B","new_string":"pass: ok","replace_all":false}]'
EDITS_MIXED_ALLOW='[{"old_string":"X_A","new_string":"pass: ok","replace_all":true},{"old_string":"X_B","new_string":"pass: ok","replace_all":true}]'
run_multiedit deny  multiedit-mixed-false-leaves-monitoring-unscored "$CURRENT_MIXED" "$EDITS_MIXED_DENY"
