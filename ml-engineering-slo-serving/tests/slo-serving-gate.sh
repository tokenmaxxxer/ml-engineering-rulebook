...
# gate's own section-placement regression (each SLO/serving element
# anchored to its own heading, not a bare mention anywhere).
set -uo pipefail
...
GATE_OFF_VAR="ML_ENGINEERING_SLO_SERVING_GATE_OFF"
...
PASS_CONTENT='## Serving pattern
online serving.

## Service SLO
p99 latency under 200ms, availability 99.9%.

## Model-behavior SLO
drift threshold on feature distribution.

## Rollout
staged rollout with canary promotion across regions.

## Rollback
if drift threshold exceeded, rollback immediately.'

FAIL_CONTENT='## Serving pattern
online serving.
...
# rollback term mentioned only under Rollout, no "## Rollback" heading of
# its own => deny.
WRONG_SECTION_CONTENT='## Serving pattern
online serving.
...
## Rollout
staged rollout with canary promotion across regions. rollback if drift threshold exceeded.'

run_write allow all-five-elements-headed  "$PASS_CONTENT"
run_write deny  rollback-section-missing  "$FAIL_CONTENT"
run_write deny  rollback-mentioned-wrong-section "$WRONG_SECTION_CONTENT"

# --- Edit, single occurrence, section-placed correctly => allow ---
run_edit allow edit-adds-rollback-section "$FAIL_CONTENT" "staged rollout with canary promotion across regions." "staged rollout with canary promotion across regions.

## Rollback
if drift threshold exceeded, rollback immediately." ""

# --- Edit with replace_all: true against a multiply-occurring old_string ---
CURRENT_MULTI='## Serving pattern
TBD
...
## Rollback
if drift threshold exceeded, rollback immediately.'
run_edit allow edit-replace-all-true  "$CURRENT_MULTI" "TBD" "online serving with staged canary promotion." "true"
run_edit deny  edit-replace-all-false "$CURRENT_MULTI" "TBD" "online serving with staged canary promotion." "false"

# --- MultiEdit, mixed replace_all true/false per edit, each independently honored ---
CURRENT_MIXED='## Serving pattern
X_A
...
## Rollback
if drift threshold exceeded, rollback immediately.'
EDITS_MIXED_DENY='[{"old_string":"X_A","new_string":"online serving with staged canary promotion.","replace_all":true},{"old_string":"X_B","new_string":"placeholder text with no drift wording","replace_all":false}]'
EDITS_MIXED_ALLOW='[{"old_string":"X_A","new_string":"online serving with staged canary promotion.","replace_all":true},{"old_string":"X_B","new_string":"drift threshold on feature distribution.","replace_all":true}]'
run_multiedit deny  multiedit-mixed-false-leaves-model-behavior-slo-unfixed "$CURRENT_MIXED" "$EDITS_MIXED_DENY"
